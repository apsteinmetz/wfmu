...
#-------------------------------------------------------------------------------------
addArtistCount<- function(dj_key,artistTokens) {
  dj_key<-artistTokens%>%summarise(count=n())%>%inner_join(dj_key)%>%arrange(desc(count))
  names(dj_key)<-c("DJ","artistCount","ShowName","onMic")
  # make DJs a factor
  dj_key$ShowName<-factor(dj_key$ShowName,as.character(dj_key$ShowName))
  dj_key$DJ<-factor(dj_key$DJ,as.character(dj_key$DJ))
  return(dj_key)
}
```
...
dj_key<-read_csv("data/dj_key.csv")
```
# Clean the Data
```{r, message=FALSE}
# 
...
dj_key<-addArtistCount(dj_key,artistTokens)
# #get rid of DJs with less than 100 artists, ever.  Probably not a music show
dj_key<-filter(dj_key,artistCount>100)
artistTokens<-semi_join(artistTokens,dj_key)
#regroup
...
gg<-ggplot(artist_count[1:20,],aes(x=artistToken,y=count/nrow(dj_key)*100))+geom_col()
...
```{r}
#make sure bars are in height order
#dj_key$DJ<-reorder(dj_key$DJ,dj_key$count)
gg<-ggplot(dj_key[1:20,],aes(x=DJ,y=artist_count))+geom_col()
