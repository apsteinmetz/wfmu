...
DJURLs<-getDJURLs()
```
If the dj_key file is already built load it.  If not run the chunk following the loading step to build it.
```{r}
#load(file='dj_key.RData')

```

Get the show names for each DJ and identify those that are not currently on the air.  Here is where we build the dj_key data frame initially.  If the file is already built, load it instead of running the chunk.
```{r}
#-------------------------------------------
...
```
Eliminate non-music DJ's from the dj_key table.  This list is created by hand and was last updated at the end of 2017.
```{r}
excludeDJs <-
  c('SD',
    'HA',
...
```
Add show counts to the dj_key table.
```{r}
