...
allDJArtists<- allDJArtists %>% filter(!(Title %in% songs_to_strip$FirstSong))
...
allDJArtists %>% 
  ...
  ggplot(aes(x=YrQtr,y=Spins))+geom_col()+facet_wrap(~ArtistToken)+scale_x_continuous()+ggtitle("Dead Artist Effect")
...
allDJArtists %>% 
  ...
  arrange(YrQtr) %>% 
  filter(YrQtr> as.yearqtr("2010-01-01")) %>% 
  ggplot(aes(x=YrQtr,y=Spins))+geom_col()+facet_wrap(~ArtistToken)+scale_x_continuous()+ggtitle("Durability of popular artists")+ylim(0,100)
