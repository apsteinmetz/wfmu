...
gg
```
So you are not a data nerd, what does this mean?  We have some idea that DJs clustered together are somewhat similar, but how?  This is not a very interesting cluster analysis as the clusters are not well define and have a lot of overlap.  It's a pretty evenly distributed cloud.  The "PC" axes have no specific attribute, by design, but we can get some intuition around them by looking at the most common artists for DJs at the extremes of each component.
```{r, message=FALSE, warning=FALSE}
