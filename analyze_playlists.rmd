...
gg
```
So what does this mean?  We have some idea that DJs clustered together are somewhat similar, but how?  The "PCs" have no specific attribute, by design, but we can get some intuition around them by looking at the most common artists for DJs at the extremes of each component.
```{r}
top_count<-5
pca<-prcomp(j) %>% tidy()
...
quadrant_wc<-function(DJs,plot_quadrant=c("n","e","s","w")){
  ...
  xy<-switch(plot_quadrant,
         ...
         'e'=c(.7,.4),
         'w'=c(.1,.3))
  songs<-songs_in_common(DJs) %>% 
    rename(word=ArtistToken,count=FaveIndex)
  gg<-gg_wordcloud(songs)
...
Sometimes, when using principal components analysis, we can ascribe clear meanings to the dimensions.  In this case it's not so clear which shows that the DJs are not overly stuck in certain genres.  Looking at the artists, we might say that higher values of PC2 are associated with old-school classic rock that tilt towards R&B.  The other end of the dimension is "not that."  PC1 is even harder to interpret.  I'd say lower values are associated with alternative rock but that is iffy.
