...
#songs_to_strip
```
Data is reasonably clean.  Do some analysis. What do we have
...
artist_count<-playlists %>% group_by(ArtistToken) %>% summarise(Artists=n()) %>% nrow() #Artists
song_count<-playlists %>% ungroup %>% transmute(paste(ArtistToken,Title)) %>% distinct() %>% nrow() #songs
...
Who are the most popular artists amoung DJs that are on the current schedule?
```{r, message=FALSE, warning=FALSE}
...
Who are the top trending artists at WFMU over the last three years?
...
artist_trends<-playlists %>% 
  ...
  nest() %>% 
  #heavy stuff here
  mutate(model = purrr::map(data, ~ lm(Spins ~ YrQtr, data = .))) %>% 
  unnest(model %>% purrr::map(broom::tidy)) %>% 
  filter(term=="YrQtr") %>% 
  filter(statistic>2.0) %>% #only statistically significant trends
  arrange(desc(estimate))

trending_artists<-artist_trends[1:10,] %>% na.omit() %>% pull(ArtistToken) 

#now we have the 150 most-played artists over the last three year where there
