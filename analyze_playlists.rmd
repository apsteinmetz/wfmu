...
year_count<-playlists %>% 
  ...
  distinct() %>% nrow()
dj_count<-playlists %>% summarise(DJs=n()) %>% nrow #DJs
...
Who are the most popular artists amoung DJs that are on the current schedule over the past five years?
```{r, message=FALSE, warning=FALSE}
...
```
How about all playlists, on an off-schedule DJs.
```{r, message=FALSE, warning=FALSE}
...
```
#Most played songs.
Everybody likes top 10 lists.  WFMU's biggest hits! Ranked by year and number of DJ's that played the song, not by by number of plays.  A single DJ can greatly skew the results.  I once asked the music director of WDUB, my college station, why The Buggles were on the hot new artist list when I was the only one who played them....Oh.
```{r, message=FALSE, warning=FALSE}

#function to calculate how much a single dj accounts for the plays of a song or how few djs do.
# 1 means only one DJ plays it.  Lower numbers are more spread around.
...
playlists_songs<-playlists %>% 
  ungroup() %>% 
  transmute(DJ=as.character(DJ),AirDate=AirDate,Song=paste(ArtistToken,Title))

DJ_count_by_song<-playlists %>%
  ungroup() %>% 
  transmute(DJ=DJ,AirDate=year(AirDate),Song=paste(ArtistToken,Title)) %>%
  distinct() %>%
  group_by(Song,AirDate) %>% 
  summarise(DJ_Count=n()) %>% 
  filter(DJ_Count>4) %>% 
  arrange(AirDate,desc(DJ_Count))
...
# how concentrated is a  song by DJ
# if the index is greater than  0.99 only one DJ has ever played the song
# if it's in the top 50 overall song plays, dump it as it will skew the top 10 lists
for (n in 1:100){
  print(song_conc(count_by_song$Song[n]))
  
}


top_10_long<-DJ_count_by_song %>% group_by(AirDate) %>% arrange(AirDate,desc(DJ_Count))
top_10<-NULL
...
```
No newer artists?  Lots of know stuff gets played on WFMU but nothing percolates into the conciousness of many DJs at once, it seems.  So "trending" seems to be irrelevant.  Despite that, let's see if we can identify similar DJs.
