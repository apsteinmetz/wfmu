...
artist_count<-playlists %>% ungroup() %>% group_by(ArtistToken) %>% summarise(Artists=n()) %>% nrow() #Artists
song_count<-playlists %>% ungroup() %>% transmute(songs=paste(ArtistToken,Title)) %>% distinct() %>% nrow() #songs
