...
```{r}
pca<-prcomp(j) %>% tidy()
top_pc1_DJs<-pca %>% filter(PC==1) %>% top_n(3) %>% pull(row) %>% as.character()
bottom_pc1_DJs<-pca %>% filter(PC==1) %>% top_n(-3) %>% pull(row) %>% as.character()
top_pc2_DJs<-pca %>% filter(PC==2) %>% top_n(3) %>% pull(row) %>% as.character()
bottom_pc2_DJs<-pca %>% filter(PC==2) %>% top_n(-3) %>% pull(row) %>% as.character()

playlists %>% 
  filter(DJ %in% top_pc1_DJs) %>% 
  group_by(DJ,ArtistToken) %>% 
  summarise(n=n()) %>%
  spread(DJ,n) %>% 
  mutate(FaveIndex=.[[2]]+.[[3]]-abs(.[[2]]-.[[3]])) #%>% 
