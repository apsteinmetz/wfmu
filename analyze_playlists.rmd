...
```{r}
  #do we want to strip the signature song the some DJs use to open every show?  Reduce it's weighting?
  #not sure. Sample code to examine it.
  allDJArtists %>% 
    filter(DJ=="TT") %>% 
    group_by(AirDate) %>% 
    summarize(FirstArtist=first(ArtistToken)) %>% 
    group_by(FirstArtist) %>% 
    summarise(FirstPlayCount=n()) %>% 
    arrange(desc(FirstPlayCount))
  
#possible way to dampen influence.  If most popular artist of a DJ
#is more than double the average of the next 10, reduce it to the number two artist count plus 1
  
  top_artists<-allDJArtists %>% 
    filter(DJ=="TT") %>%
    group_by((ArtistToken)) %>% 
    summarise(play_count=n()) %>% 
    arrange(desc(play_count))

    top_artists$play_count[1]/mean(top_artists$play_count[2:11])
```
...
allDJArtists %>% 
  ...
  summarise(Spins=n()) %>% 
  arrange(YrQtr) %>% 
  filter(YrQtr> as.yearqtr("2010-01-01")) %>% 
  ggplot(aes(x=YrQtr,y=Spins))+geom_col()+facet_wrap(~ArtistToken)+scale_x_continuous()

```

#Durability of popular artists
```{r}
big_artists=c('nrbq','beachboys','stones','beatles','kinks','dylan')
...
```
So, like, what was with the Beach Boys in 2015.  They went on tour but who started playing them?
```{r}
allDJArtists %>% 
  ungroup() %>% 
  filter(ArtistToken=='beachboys') %>% 
  mutate(YrQtr=as.yearqtr(AirDate))  %>% 
  group_by(DJ,YrQtr) %>% 
  summarise(Spins=n()) %>% 
  arrange(YrQtr) %>% 
  filter(YrQtr> as.yearqtr("2015-01-01")) %>% 
  arrange(desc(Spins))
```
