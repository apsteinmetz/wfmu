...
gg
```
So what does this mean?  We have some idea that DJs clustered together are somewhat similar, but how?  The "Components" have no specific attribute, by design, but we can get some intuition around them by looking at the most common artists for DJs at the extremes of each component.
```{r}
pca<-prcomp(j) %>% tidy()
