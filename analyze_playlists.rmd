...
djCorpus <- Corpus(VectorSource(concat_artists$Artists))
...
plotNetwork <- function(docMatrix) {
  ...
  m2<-as.matrix(djdtm)
  rownames(m2)<-concat_artists$DJ[which(concat_artists$onMic==TRUE)]
...
# # don't try to display concat_artists. It's a monster and will hang machine!
# concat_artists<-combineAllArtists()
# save(concat_artists,file="concat_artists.RData")
# load("concat_artists.RData")
# 
...
for (i in 1:length(djCorpus)) {
  ...
  meta(djCorpus[[i]], tag="id") <- concat_artists$DJ[i]  
  meta(djCorpus[[i]], tag="DJ") <- concat_artists$DJ[i]
  meta(djCorpus[[i]], tag="onMic") <- concat_artists$onMic[i]
}
