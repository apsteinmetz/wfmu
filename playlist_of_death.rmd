...
death_tokens<-death_tokens %>% mutate(death_flag=TRUE)
...
play_count<-left_join(play_count,death_tokens) %>% 
  select(-Date) %>% 
  mutate_cond(is.na(death_flag),death_flag=FALSE)
```
...
Perhaps we should advance the death dates by a week to determine the quarter they will fall in.
```{r}
...
play_count %>% ungroup() %>% filter(ArtistToken=="BobbyWomack",z_score>0)
```
That fixed it.
