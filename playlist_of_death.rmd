...
save(death_page,file="death_page_html.rdata")
...
```{r}
model <- glm(death ~ z_score + dj_concentration,
...
rbind(prop.table(table(play_train_label),prop.table(table(play_train_label))))
death.knn<-knn(play_train,play_test,play_test$death,k=3)
summary(death.knn)

```
We are generating positive values, at least.  Are they any good?
```{r}
table(play_count_norm$death,death.knn)
confusionMatrix(table(play_count_norm$death,death.knn))
```


try another approach.  How about Baysian, where we condition a prediction of death upon the play count being an outlier?
