...
Let's rebuild and add a feature to our data set that includes the number of DJs who played the artist in any quarter and how many times the average number of DJs who played the artist.
```{r}
dj_counts<-playlists %>% 
  ungroup() %>% 
  mutate(AirDate=as.yearqtr(AirDate))  %>% 
...
```{r}
play_count_z2 %>% filter(dj_multiple>7)
```
...
```{r}
play_count_z2 %>% filter(z_score>6.5)
```
