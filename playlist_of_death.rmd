...
create list of name/band tokens for each row
```{r}
#take at most three band names.
deaths<-deaths %>% separate(Band,into=c("band1","band2","band3"),sep=", ",extra="drop")

#we don't acutally care what artist is in what band, just that we have a token and a yearqtr variable for each.
#use tidyr to gather the tokens
death_tokens<-deaths %>% 
  select(-Age,-Location,-Cause_of_Death,-Date) %>% 
  gather(key="key",value="ArtistToken",-AirDate) %>% 
  select(-key) %>% 
  filter(!is.na(ArtistToken))
#tokeniize

tokenize_artist<-function(artist){
