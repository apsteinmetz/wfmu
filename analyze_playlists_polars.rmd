...
playlists <- polars::pl$read_parquet("data/playlists.parquet")

DJKey <- polars::pl$read_parquet('data/DJKey.parquet')
...
# polars function to simulate yrqtr type
...
dj_count<-playlists %>% 
  ...
  mutate(yrqtr = pl_to_yrqtr(AirDate))  %>% 
  group_by(DJ,yrqtr) %>% 
  summarise(Spins=n()) %>% 
  group_by(yrqtr) %>% 
  summarise(DJCount=n()) |> 
