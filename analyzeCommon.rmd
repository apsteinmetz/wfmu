...
# this combines all the artist tokens into one space-delmited string
#which can then be used as a document corpus for the text mining package
...
#get list of all artists and frequency
#take a stab at fixing minor typos
#warning this is SLOW. You will get sensible results even if you ignore this step
allDJArtists$artist<-fix_spelling(allDJArtists$artist)
...
```
Run a spell check on artist names to find letter transpositions.
It works by assuming the most frequently cited version is the correct one.
```{r}
