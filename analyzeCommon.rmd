...
```
So love is popular what are the other popular words in band names?  Limit ourselves to words of more than three characters.
```{r}
allDJArtists$artist%>%
  str_split(" ")%>%
  unlist()%>%
...
temp%>%filter(str_length(band_word)>3)->wordCount
wordCount
...
```{r}
wordcloud(words = wordCount$band_word, freq =                                wordCount$count,max.words=maxWords,                                  random.order=FALSE,rot.per=0.35, 
          colors=brewer.pal(8, "Dark2"),scale = c(3,.3))
