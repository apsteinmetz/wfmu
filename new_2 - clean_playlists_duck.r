...
playlists$ArtistToken <- str_replace(playlists$ArtistToken, "Uknown", "Unknown")
...
playlists <- filter(playlists, !str_detect(Artist, "^Wake "))
playlists <- filter(playlists, !str_detect(Artist, "^Pidge "))
playlists <- filter(playlists, !str_detect(Title, "^The Pidge"))
playlists <- filter(playlists, !str_detect(Artist, "^Clay "))
...
song_conc <- function(song) {
  g <- playlists %>%
    ...
    pull(n) %>%
    c(rep(0, NUM_DJS)) %>%
    .[1:NUM_DJS] %>% #pad to include no-play DJs in Gini calc
    ineq::Gini()
  return(g)
}
