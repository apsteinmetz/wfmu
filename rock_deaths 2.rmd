...
deaths_loc<-deaths_loc %>% mutate_cond(country=="",country="Not Specified")
deaths_loc<-deaths_loc %>% mutate_cond(is.na(city),city="Not Specified")
deaths_loc<-deaths_loc %>% mutate_cond(country=="U.S.",country="USA")
deaths_loc<-deaths_loc %>% mutate_cond(country=="U.K.",country="England")
...
city_pop<-city_pop_raw %>% 
  ...
  mutate(city=str_replace(city,"([A-Za-z -]+) city|town|municipality|CDP|village|borough","\\1")) %>%
...
Now we have a clean list of cities and populations.  Let's merge it with our death locations to get a per-capita number.  We arbitrarily limit our cities to those with populations greater than 200k to avoid skewed results.
```{r}
...
#geocode lat/lon from city names.  Source is "dsk."  Google API didn't handle list
#of cities without claimng data limit reached.  Seems there is a limit
#on queries per second as well as absolute number.
#does not seem to be vectorized

get_lat_lon<-function(city,state){
  return(geocode(paste(city,state,sep=","),
                  source="google",output="latlona"))

}

mytest<-function(city,state){
  return(data_frame(lon=str_length(city),lat=str_length(state)))
}

lapply(deaths_city, function(x) print(x$city))

latlon_d<-geocode(paste(deaths_city$city,deaths_city$state,sep=","),
                  source="dsk",output="latlona")
latlon_d<-latlon_d %>% separate(address,into = c("city","state"),sep=",")

deaths_city<-bind_cols(deaths_city,latlon_d)
#get basmap
#us_states_map<-states() %>% as_data_frame()
