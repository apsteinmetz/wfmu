...
I got inspired to look at this while reading about the`tidycensus` package.  Try as I might, though, I could not get the national city-level population data.  I conclude that the functionality of this package is too limited, for now.  I reverted to the excellent `censusapi` package.  This still took a lot of trial and error before I could craft a query that returned what I needed.  
```{r}
library(censusapi)
#I've stored my API key as an environment variable
city_pop<-as_data_frame(getCensus(name="acs3",
                                  vintage=2012,
                                  vars=c("NAME","B01001_001E"),
                                  region="place"))

city_pop %>% select(-state,-place) %>% separate(NAME,into=c("city","state"),sep=",",remove=FALSE)
