...
```
Quite a few.  I should add I don't have much confidence that the `dplyr` chain I used above is an efficient way to get this answer.  You, the experienced R user, might have a better approach.  Remember Ashley Madison, the hookup site for cheaters that was hacked?  Itturns out that "Ashley" and "Madison" are both highly fluid in gender assignment.  Was that deliberate?
...
You can learn more about this cool package [here](https://ropensci.org/tutorials/gender_tutorial/).

Now back to our rock musician problem.  First we have to separate the name into pieces.  BUT, where there are three or more words in the name, which is the first name?  Joe Harmonica Watson and Wild Bill Williams both have nicknames but different positions for the first name.
...
temp<-genderdata::ssa_national %>% 
  #filter(name %in% c("adam","ashley","madison")) %>%
  mutate(gender=ifelse(male<female,"female","male")) %>% 
  select(name,gender) %>% 
  group_by(name,gender) %>% 
  summarise(count=n()) %>% 
...
We need to visualize this in a chart form. Let's show some bad, but typical, versions.  The chart below is the kind of thing I often see in presentations in my organization.
```{r}
...
Could be worse. The categories could be in alphabetical order.  There are LOTS of [questions and answers on this topic](https://stackoverflow.com/questions/3253641/change-the-order-of-a-discrete-x-scale) on Github.com.  For my money, `as_factor` from the `forcats` package (loaded as part of the `tidyverse`) is the simplest way to address this, as shown above.
...
```
Ugh! These candy jars are unusable.  I include them because I see this sort of plot all the time. People don't naturally gravitate to the far more useful scatter plot.
```{r}
