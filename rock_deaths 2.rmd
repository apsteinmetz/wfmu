...
city_loc<-geo_data %>% rename(STUSPS=state) %>% 
  left_join(us_states_map) %>%
  transmute(city=city,st=STUSPS,state=NAME,lat=latitude,lon=longitude) %>% 
  group_by(city,state) %>% 
  top_n(1,lat)%>% 
  top_n(1,lon) %>% 
  distinct()
...
#   return(bind_cols(city=city,
#                    state=state,
#                    geocode(paste(city,state,sep=","),
#                            source="dsk",output="latlon")))
# 
# }
# 
# latlon_d<-deaths_city  %>% 
#   split(.$city) %>% 
#   map(function(x)get_lat_lon(x$city,x$state)) %>% 
#   bind_rows()
