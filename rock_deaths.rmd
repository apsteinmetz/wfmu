...
That's the stereotype for rockers, or it was.  We only need to look at Keith Richards, over 70 and going strong, to find a major counterexample.  Do rockers die young?  What do they die of?  How does that compare to the broader population (in the U.S., anyway).

Wikipedia maintains a table of [deaths in rock](https://en.wikipedia.org/wiki/List_of_deaths_in_rock_and_roll).  Let's scrape that list to make some generalizations.  We use the `tidyverse` dialect throughout, just so you know.
...
```
A further wrinkle is some of the age-at-death numbers are given as a range like "45-46" which causes `html_table` to create a `character` column while it will otherwise be of type `numeric` if it doesn't see it.  We want to use `bind_rows` to combine all the web page tables into one data frame but it will break on the type conflict.  Let's arbitrarily take the lowest number in age range before binding.  We can do this as part of the table extraction process.  Also, omit the first five tables which, by inspection, we know are not the mortality tables.
...
This gives us a decent picture of how rock stars die.  Mostly from the same things that kill all of us. Heart disease and cancer are the leading causes of death in US, followed by lung disease and accidents according to https://www.cdc.gov/nchs/fastats/deaths.htm and shown below.
...
```
We see that the `gapminder` data is pretty linear so we should be okay to extrapolate.  That said, we are going way beyond the observed range.  I saw some sources that indicate the line should bend downward in the 1800s.  I am not controlling for demographics, either.  Our sample sizes are pretty small at the far end of the range, anyway, so any conclusions would have to be taken with a huge grain of salt,regardless.  Still, lets plunge on!
...
The series for rock musicians in the chart below is very unstable early on because the sample size is so small.
...
How do causes of death change as rock musicians age?  Again, we have to deal with small sample sizes so let's aggregate in 5-year buckets.
```{r}
# bin by 5 year groups
breaks<-c(0,25,30,35,40,45,50,55,60,65,70,75,80,85)

cause_by_age<-deaths %>% 
  filter(Age>19,Age<86) %>% 
  transform(bin=cut(Age,breaks=breaks)) %>% 
  group_by(bin,General_Cause) %>% 
  summarise(Count=n()) %>% 
  group_by(bin) %>%
  mutate(prop=Count/sum(Count)) %>% 
  arrange(bin,desc(Count)) %>% 
  #top_n(n=5,wt=Count) %>% 
  {.}

#restore continuous age variable
cause_by_age<-cause_by_age %>% 
  select(bin) %>% 
  unique() %>% 
  bind_cols(data_frame(Age=breaks[2:14])) %>% 
  right_join(cause_by_age)

gg<-cause_by_age %>% 
  ggplot(aes(Age,prop,fill=General_Cause,color=General_Cause))+geom_col(width=5)
...
If you make it into your 40s as a rocker you are out of the  woods for suicide murder and accidents. Cancer supplants accidents as the leading cause after age 40.  Heart disease is prevalent starting at 40 and onward.  "Not Specified" rise substantially with age.  I suspect it's because rock stars who die in their prime are more reported upon.

Let's visualize the same data as a smoothed density plot, just to experiment with another view.  Which do you prefer?  It's certainly accomplished with fewer lines of code!

```{r}
#filter top causes to reduce clutter
#cause_by_age<-deaths %>% filter(General_Cause %in% cause_table$General_Cause[1:15])
cause_by_age<-deaths %>% mutate_cond(!(General_Cause %in% cause_table$General_Cause[1:15]),General_Cause="Other")
gg<-cause_by_age %>% 
