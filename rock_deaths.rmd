...
Let's use this data to question our "live fast" assertion from above.  We know deaths from all causes rise with age.  So is dimunition of deaths by accident as age increases just a reflection of other categories growing in relation? As a reality check, let's ask if the absolute number changes with age.

```{r}
gg<-deaths %>% filter(General_Cause=="Accident") %>% 
  ggplot(aes(Age,fill="red"))+geom_histogram(bins=10)
gg<-gg+labs(title='Number of Reported Deaths by "Accident" by Age',
            x='Age at Death',
            y="Count")
gg<-gg+geom_histogram(data=deaths,aes(Age,fill="black"),bins=10,alpha=0.2)
gg<-gg+ scale_fill_manual(name = 'Lines', 
         values =c('black'='black','red'='red'), labels = c('Accident','Everything'))
gg
```
So, yes, rock musicians do get more careful as they age.
