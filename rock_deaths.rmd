...
We know deaths from all causes rise with age.  Do the number of deaths from accidents merely shrink relative to other causes with age? a reality check, let's ask if the absolute number changes with age.

```{r}
deaths %>% filter(General_Cause=="Accident") %>% 
  ggplot(Age)+geom_histogram(bins=10)
```
